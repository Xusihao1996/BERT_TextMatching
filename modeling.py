  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("encoder"):
        ...
        (self.all_encoder_layers, self.attention_output) = transformer_model(
            input_tensor=self.embedding_output,
...
def embedding_postprocessor(input_tensor,
  ...
  if use_position_embeddings:
    ...
    with tf.control_dependencies([assert_op]):
      ...
      left = tf.roll(output, shift=[2, -2, -1], axis=[1, 1, 1])
      bi_output = output + left
      left_twice = tf.roll(left, shift=[2, -2, -1], axis=[1, 1, 1])
      tri_output = bi_output + left_twice
      output_final = tf.concat([output, bi_output, tri_output], axis=1)

  output = layer_norm_and_dropout(output_final, dropout_prob)
  return output
...
def create_attention_mask_from_input_mask(from_tensor, to_mask, one_mask):
  ...
  broadcast_ones = tf.tile(broadcast_ones, (1, 3, 1))
    ...
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.variable_scope("attention"):
        ...
        with tf.variable_scope("self"):
          (attention_head, attention_probability) = attention_layer(
